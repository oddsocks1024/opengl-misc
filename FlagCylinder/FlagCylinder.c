...
void LoadGLTextures(void)
{
    ...
    tex_fnames[0] = "australianflag.bmp";
    tex_fnames[1] = "unionflag.bmp";
    tex_fnames[2] = "canadianflag.bmp";
    tex_fnames[3] = "newzealandflag.bmp";
    tex_fnames[4] = "germanflag.bmp";
    tex_fnames[5] = "englandflag.bmp";
...
void DrawGLScene()
{
    ...
    glBindTexture(GL_TEXTURE_2D, texture[current_tex]);   // choose the texture to use.
    ...
    gluCylinder(quadratic, 1.0f, 1.0f, 1.0f, 128, 128);
...
gluQuadricNormals(quadratic, GLU_SMOOTH);	// Create Smooth Normals ( NEW )
    ...
    SDL_WM_SetCaption("Ian Chapman : Flag Cylinder", NULL);
            ...
            while ( SDL_PollEvent(&event) )
            {
                ...
                if ( event.type == SDL_KEYDOWN )
                {
                    switch( event.key.keysym.sym )
                    {
                        ...
                        case SDLK_SPACE:
                            current_tex++;
                            if (current_tex > (N_TEXTURES - 1)) { current_tex = 0; }
                            break;
                    } 
